node {
   def mvnHome
  stage('Prepare') {
      git url: 'https://github.com/81062/sonar-project.git', branch: 'main'
      mvnHome = tool 'maven'
   }
  stage ('Code Quality') {
  ...
  stage ('Deliver & Deployment') {
      sh 'curl -u admin:redhat@123 -T target/**.war "http://54.249.100.182:8080/manager/text/deploy?path=/kesav&update=true"'
  }
  stage ('SmokeTest') {
      sh 'curl --retry-delay 10 --retry 5 "http://3.112.24.157:8080/kesav"'
  }
}
